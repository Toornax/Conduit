...
//! | [`modes`] | `KSPROPERTY_AUDIOSIGNALPROCESSING_MODES` au-dessus de [`property`] : trait [`SignalModes`], gestionnaire [`SignalProcessingModes`] — les modes de traitement du signal des broches de **flux** wave, `DEFAULT` et lui seul (M1b-15) |
...
pub use status::{
    STATUS_BUFFER_OVERFLOW, STATUS_BUFFER_TOO_SMALL, STATUS_DATA_LATE_ERROR, STATUS_DATA_OVERRUN,
    STATUS_DEVICE_NOT_READY, STATUS_INVALID_DEVICE_REQUEST, STATUS_NO_MATCH, STATUS_NOT_FOUND,
    STATUS_NOT_SUPPORTED, STATUS_PRIVILEGE_NOT_HELD,
};
pub use stream::{
