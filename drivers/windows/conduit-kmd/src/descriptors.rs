...
//!   table d'automatisation qui ne porte, depuis M1b-15, qu'une seule propriété —
...
//!
//! # Le mode de traitement du signal (M1b-15, driver-design.md §4.4)
//!
...
//! l'expérience qu'elle sert est décrite dans driver-design.md §4.4.
//!
...
use portcls::{
    ...
    jack_description_item, jack_info_change_item, mute_item, packets_item,
    signal_processing_modes_item, transport_item, version_item, volume_item, with_events,
};
use portcls_sys::{
...
/// depuis M1b-15 — la plage, puis la `KSATTRIBUTE_LIST` qu'elle annonce.
///
...
// ---------------------------------------------------------------------------------
// La liste d'attributs des broches de flux (M1b-15, driver-design.md §4.4).
//
...
///
/// `automation` n'est plus vide depuis M1b-15 : elle porte
...
/// `KSPROPERTY_AUDIOSIGNALPROCESSING_MODES` du filtre `WaveRender<n>` (M1b-15).
///
...
/// serait un pilote qui n'a jamais l'air « mode aware » — exactement la panne que M1b-15
/// cherche à écarter.
...
/// Table d'automatisation du filtre `TopoRender<n>` : les cinq propriétés, **plus**
...
pub enum PinMismatch {
    ...
    /// traitement du signal, ou pas là où KS la cherche (M1b-15).
    SansAttribut(usize),
...
fn declared_by_pin(filter: &'static PCFILTER_DESCRIPTOR, pin: ULONG) -> Option<(ULONG, ULONG)> {
    // La **première** entrée du tableau : depuis M1b-15 une entrée sur deux est une liste
...
/// d'attributs à un élément (M1b-15).
///
...
/// reste exactement aussi peu « mode aware » qu'avant M1b-15 — mêmes endpoints, même son,
...
pub fn check_cable_pins(cable: u32) -> Result<(), PinMismatch> {
    ...
    for (filtre, broche) in [
        ...
        // La condition SYSVAD de M1b-15, vérifiée là où PortCls la lira : chaque plage
...
const fn range_is(
    ...
    range.MaximumChannels == channels
        ...
        // `KSDATARANGE_ATTRIBUTES` et lui seul (M1b-15) : sans ce drapeau, KS ignorerait
...
const _: () = {
    // Les deux tables de filtre **wave** (M1b-15) : une propriété — les modes de traitement
    ...
    // (doublet `const` puis `static`) : le jack, l'état, la version, les compteurs et le
    // transport, **plus** l'événement `JACKINFOCHANGE`. C'est ce dernier qui sépare un
    // endpoint qui suit l'état du câble d'un endpoint figé au démarrage.
    assert!(automation_is_well_formed(
    ...
    // Le transport non plus : c'est une observation de ce que le moteur audio a demandé, et
    // une observation ne se règle pas. Déclarer `SET` laisserait croire qu'on peut faire
    // changer d'avis le moteur sur son mode d'allocation depuis l'espace utilisateur.
    assert!(RENDER_FILTER_ITEMS[4].Flags == TRANSPORT_ACCESS_FLAGS);
    assert!(CAPTURE_FILTER_ITEMS[4].Flags == TRANSPORT_ACCESS_FLAGS);
    assert!(TRANSPORT_ACCESS_FLAGS & portcls_sys::KSPROPERTY_TYPE_SET == 0);
    // Les quatre entrées du jeu privé portent bien quatre `Id` distincts : PortCls sert la
    ...
    assert!(RENDER_FILTER_ITEMS[1].Id != RENDER_FILTER_ITEMS[4].Id);
    assert!(RENDER_FILTER_ITEMS[2].Id != RENDER_FILTER_ITEMS[3].Id);
    assert!(RENDER_FILTER_ITEMS[2].Id != RENDER_FILTER_ITEMS[4].Id);
    assert!(RENDER_FILTER_ITEMS[3].Id != RENDER_FILTER_ITEMS[4].Id);
    assert!(CAPTURE_FILTER_ITEMS[1].Id != CAPTURE_FILTER_ITEMS[2].Id);
    assert!(CAPTURE_FILTER_ITEMS[1].Id != CAPTURE_FILTER_ITEMS[3].Id);
    assert!(CAPTURE_FILTER_ITEMS[1].Id != CAPTURE_FILTER_ITEMS[4].Id);
    assert!(CAPTURE_FILTER_ITEMS[2].Id != CAPTURE_FILTER_ITEMS[3].Id);
    assert!(CAPTURE_FILTER_ITEMS[2].Id != CAPTURE_FILTER_ITEMS[4].Id);
    assert!(CAPTURE_FILTER_ITEMS[3].Id != CAPTURE_FILTER_ITEMS[4].Id);
