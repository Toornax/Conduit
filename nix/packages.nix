{ inputs, ... }:
{
  perSystem = { pkgs, system, lib, ... }:
    ...
    in
    {
      ...
      apps = {
        conduitd = { type = "app"; program = "${mkBin "conduitd"}/bin/conduitd"; };
        conduitctl = { type = "app"; program = "${mkBin "conduitctl"}/bin/conduitctl"; };
        default = { type = "app"; program = "${mkBin "conduitd"}/bin/conduitd"; };
      };

      # Exposé aux checks.
      _module.args.conduitCrane = { inherit craneLib commonArgs cargoArtifacts src; };
    };
}
